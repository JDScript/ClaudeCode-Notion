...
The top-level anchor is the Research Space page (page ID: `2306f2eb-947e-805e-9064-ed452a446342`). All research project pages are created as direct children of this page. Each project page follows the Functional Reorientation structure: a two-column header with a Project Overview callout and a Quick Links callout, an inline Tasks database, an inline References database, and a Meeting Notes sub-page. The `scaffold-project` skill creates this structure; the other skills navigate it by fetching the project page and extracting database URLs from the rendered content.
